...
La JamStack, c'est une nouvelle façon de voir la conception de site Internet, cette nouvelle façon de faire va nous permettre de réduire les couts d'hébergement et bénéficier des avantages du Cloud (CI/CD, compression des assets, FAAS…).
...
Nous allons donc déployer un site « pour tester Netlify », si vous n'avez pas de site statique sous la main, je vous propose d'en créer un rapidement avec [ViteJS](https://vitejs.dev/) :
...
::: danger Un instant
⚠️ Avant d'aller plus loin, je vous laisse tester ce que vous allez déployer.
:::
...
Je vous laisse modifier votre site Internet. Si vous avez correctement configuré votre stack applicative, vous devriez normalement déclencher une recompilation de votre site web.
...
Eh bien avec Netlify c'est possible ! Et c'est très simple ! Nous allons le configurer ensemble et tester.
...
Tester en prod, personne n'aime ça… Pourtant vous pourriez vouloir montrer votre travail à votre client / collègue sans forcément le présenter sur votre ordinateur. Avec Netlify c'est possible. Nous allons le configurer sur votre projet.
