---
description: L'objectif de cette application va être la mise en pratique de la communication Client et Serveur d'une application, mais également les à-côtés de celle-ci (Serveur, Base de données, etc.).
---

# Application Client / Serveur

L'objectif de cette application va être la mise en pratique de la communication Client et Serveur d'une application, mais également les à-côtés de celle-ci (Serveur, Base de données, etc.). Cette application couvrira l'ensemble des compétences vues ensemble :
...
- Identifier les clients (via un ID, ou un UUID, ou un TOKEN).
  ...
  - Les clients seront identifiés lors des appels réseau par l'ID / UUID ou Token en fonction du choix.
...
L'échange entre le client et le serveur n'est à faire qu'une seule fois. Vous devez sauvegarder dans l'application que celle-ci est maintenant « appairée » avec le serveur, suivant l'UUID / TOKEN / ID choisi ; cet appairage **doit être** sauvegardé dans les paramètres de l'application et doit être restauré à chaque démarrage de celle-ci.
:::
...
La collecte devra être faite à intervalle régulier (~60 secondes), celle-ci a pour but de collecter un maximum d'informations depuis les capteurs du smartphone :
...
**Vous avez les playservices ?** si oui, vous pouvez même savoir si la personne marche / court / roule en voiture.
...
- **Bonus** la vue peut être découpée en deux tabs afin d'avoir une vue listant l'ensemble des collecteurs de données connus par le serveur. Chaque ligne contiendra une action permettant d'afficher un « client précis » (donc sans connaitre son code préalablement).
